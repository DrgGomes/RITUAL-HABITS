...
import type { User as FirebaseUser } from 'firebase/auth'; // O "type" resolve o erro
...
const PHASES = [
  {
    ...
    neuroChange:
      'A dopamina caiu drasticamente. A irritação é normal, é o cérebro pedindo a droga. Respire.',
    icon: ShieldAlert,
  },
  {
    id: 'mine_field',
    ...
    neuroChange:
      "Os receptores começam a 'desgrudar'. Você sente clareza, mas gatilhos antigos ainda são fortes.",
    icon: Crosshair,
  },
  {
    id: 'fortress',
    ...
    neuroChange:
      'Novas sinapses estão se formando. O hábito antigo está fisicamente perdendo conexão.',
    icon: Anchor,
  },
  {
    id: 'new_normal',
    ...
    neuroChange:
      'Homeostase atingida. A disciplina deixou de ser esforço e virou natural.',
    icon: Crown,
  },
];
...
export default function App() {
  ...
  const currentPhase =
    PHASES.find((p) => stats.streakDays <= p.maxDays) ||
  ...
  const phaseProgress =
    (Math.min(stats.streakDays, currentPhase.maxDays) / currentPhase.maxDays) *
    100;
  ...
  const handleDailyCheckIn = () => {
    ...
    if (shouldShowConfetti) {
      ...
      setTimeout(
        ...
        currentPhase.id === 'war_zone' ? 3000 : 8000
      );
    } else {
      setShowRipple(true);
    ...
    const newLog = [
      { date: today, type: 'success' as const, note: `Dia ${newStreak}` },
      ...historyLog,
    ];
  ...
  const handleRelapse = () => {
    const today = new Date().toDateString();
    const newLog = [
      { date: today, type: 'relapse' as const, note: 'Ajuste de rota.' },
      ...historyLog,
    ];
      ...
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 relative overflow-hidden">
        ...
        >
          ...
          <p className="text-slate-400 mb-8 text-sm">
            Assuma o controle da sua dopamina. Gamifique sua liberdade.
          </p>
          ...
          <p className="text-xs text-slate-500 mt-4">
            Seus dados salvos na nuvem, seguros e privados.
          </p>
        </motion.div>
      </div>
    );
  ...
  return (
    <div
      className={`min-h-screen font-sans flex flex-col items-center justify-start p-4 pb-28 relative overflow-hidden transition-colors duration-1000 ${
        stats.isDamaged ? 'bg-zinc-900 grayscale' : 'bg-slate-900'
      } text-white`}
    >
      <div
        className={`absolute top-0 left-0 w-full h-full overflow-hidden -z-10 transition-opacity duration-1000 ${
          stats.isDamaged ? 'opacity-0' : 'opacity-100'
        }`}
      >
        <div
          className={`absolute top-0 left-0 w-full h-full bg-gradient-to-br ${currentPhase.bg} opacity-20`}
        ></div>
      </div>
          ...
          >
            <header className="text-center space-y-2 relative">
              <button
                ...
                className="absolute right-0 top-0 w-8 h-8 rounded-full overflow-hidden border border-white/20 hover:border-white/50 transition-colors bg-slate-800 flex items-center justify-center"
              >
                {user.photoURL ? (
              ...
              <div className="inline-flex items-center gap-2 bg-slate-800/80 px-4 py-1.5 rounded-full border border-slate-700/50 backdrop-blur-md">
                ...
                ></div>
                <span
                  className={`text-xs font-bold uppercase tracking-widest ${currentPhase.color}`}
                >
              ...
              <h1 className="text-6xl font-black text-white tracking-tighter drop-shadow-2xl">
                Dia{' '}
                {stats.streakDays +
                  (lastCheckIn === new Date().toDateString() ? 0 : 1)}
              </h1>
            ...
            <div className="bg-slate-800/40 border border-slate-700/50 rounded-3xl p-6 relative overflow-hidden">
                ...
                <div>
                  ...
                  <span className="text-xl font-bold text-white">
                    ...
                    <span className="text-slate-500 text-sm font-normal">
                      de {currentPhase.maxDays} dias
                    </span>
                  </span>
                </div>
                <PhaseIcon
                  className={`${currentPhase.color} opacity-80`}
                  size={28}
                />
              </div>
              ...
              <p className="text-xs text-slate-500 mt-3 relative z-10">
                {stats.isDamaged
                  ? 'Reinicie a contagem para voltar à fase.'
                  : 'Você já passou pela parte mais instável.'}
              </p>
            </div>
                ...
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-5 flex gap-4 items-start">
                  ...
                  <div>
                    <h4 className="text-white font-bold text-sm mb-1">
                      Ajuste Neural em Andamento
                    </h4>
                    <p className="text-slate-400 text-sm leading-relaxed">
                      {currentPhase.neuroChange}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="relative z-10">
                ...
                <p
                  className={`text-xs font-bold uppercase tracking-wider ${
                    stats.isDamaged ? 'text-red-200' : 'text-emerald-100'
                  }`}
                >
            ...
            <header className="pb-4 border-b border-slate-800 flex justify-between items-center">
              <div>
                ...
                <p className="text-slate-400 text-sm">
                  Gestão deliberada do protocolo.
                </p>
              </div>
              <button
                onClick={handleLogout}
                ...
                <div className="mt-2 space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 bg-emerald-600 hover:bg-emerald-500 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-1"
                    >
                      <Save size={14} /> Salvar
                    </button>
                    <button
                      onClick={() => setIsEditingHabit(false)}
                      className="flex-1 bg-slate-700 hover:bg-slate-600 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-1"
                    >
                      <X size={14} /> Cancelar
                    </button>
                  </div>
                  <p className="text-[10px] text-orange-400 flex items-center gap-1 mt-1">
                    <AlertTriangle size={10} /> Mudanças frequentes dificultam a
                    consolidação neural.
                  </p>
                </div>
              ) : (
      ...
      {showRelapseModal && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-950/90 backdrop-blur-sm p-4">
          <motion.div
            ...
            className="bg-slate-900 border border-slate-700 p-6 rounded-2xl max-w-xs w-full shadow-2xl"
          >
            <h3 className="text-white font-bold text-lg mb-2">
              Ajuste de Rota
            </h3>
            <p className="text-slate-400 text-xs mb-6 leading-relaxed">
              Sem culpa. O sistema manterá seu Nível e Dias Totais.
              Reiniciaremos apenas a sequência atual.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setShowRelapseModal(false)}
                className="flex-1 py-3 bg-slate-800 rounded-xl text-white text-sm font-bold"
              >
                Voltar
              </button>
              <button
                onClick={handleRelapse}
                className="flex-1 py-3 bg-red-500/10 text-red-500 border border-red-500/20 rounded-xl text-sm font-bold"
              >
                Confirmar
              </button>
            </div>
